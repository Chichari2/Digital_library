...
def home():
    ...
    if sort_by == 'author':
        books = Book.query.join(Author).filter(Book.title.ilike(f'%{search_query}%')).order_by(Author.name, Book.title).all()
    elif sort_by == 'publication_year':
        books = Book.query.filter(Book.title.ilike(f'%{search_query}%')).order_by(Book.publication_year, Book.title).all()
    else:
